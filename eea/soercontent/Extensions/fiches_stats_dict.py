...
def hasDoneTransition(history,transition_string):
    """ Given history dictionary it returns true if it finds a certain transition string in transition title.
    """
...
def fichesStatsDict(context):
	...
	#        member = acl_users.getUserById(userid)
		...
		#count_sentences = context.count_sentences(textclean)
		#count_words = context.count_words(textclean)
		#flesch_reading_ease_score = context.flesch_kincaid_reading_ease(textclean)
		#flesch_kincaid_grade_level = context.flesch_kincaid_grade_level(textclean)
		...
		nr_ind = 0
		nr_ind_draft = 0
	#DISABLED due to permission issues on certain unpublished objects for SOERCopyEditors group   
